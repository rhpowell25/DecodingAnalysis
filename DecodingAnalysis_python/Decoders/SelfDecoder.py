...
class Self_Decoder():
    ...
    def __init__(self, xds, predict_what, Zero_Factor, Norm_Factor, reserve_trials):
        ...
        if reserve_trials == 'Yes':
            training_spikes, training_Behavior = [[] for ii in range(cv_folds)], [[] for ii in range(cv_folds)]
            test_spikes, test_Behavior = [[] for ii in range(cv_folds)], [[] for ii in range(cv_folds)]
            
            formatted_training_spikes, formatted_training_Behavior = [[] for ii in range(cv_folds)], [[] for ii in range(cv_folds)]
            ...
            H_baseline = [[] for ii in range(cv_folds)]
            
            r2_values_train = np.zeros([sorted_Behavior[0].shape[1], cv_folds])
            r2_values_test = np.zeros([sorted_Behavior[0].shape[1], cv_folds])
            vaf_values_train = np.zeros([sorted_Behavior[0].shape[1], cv_folds])
            vaf_values_test = np.zeros([sorted_Behavior[0].shape[1], cv_folds])
            
            predicted_Behavior_train, predicted_Behavior_test = [[] for ii in range(cv_folds)], [[] for ii in range(cv_folds)]
        else:
            training_spikes, training_Behavior = [], []
            
            formatted_training_spikes, formatted_training_Behavior = [], []
            
            H_baseline = [[] for ii in range(1)]
            
            r2_values_train = np.zeros([sorted_Behavior[0].shape[1], 1])
            vaf_values_train = np.zeros([sorted_Behavior[0].shape[1], 1])
            
            predicted_Behavior_train = []
        ...
        if reserve_trials == 'Yes':
            ...
            for train_index, test_index in kf.split(np.arange(0, trial_lim)):
                ...
                training_spikes[cv] = [x for ii, x in enumerate(sorted_spikes) if ii in train_index]
                test_spikes[cv] = [x for ii, x in enumerate(sorted_spikes) if ii in test_index]
                training_Behavior[cv] = [x for ii, x in enumerate(sorted_Behavior) if ii in train_index]
                test_Behavior[cv] = [x for ii, x in enumerate(sorted_Behavior) if ii in test_index]
                test_indices[cv] = test_index
                cv += 1
        else:
            training_spikes = sorted_spikes
            training_Behavior = sorted_Behavior
        ...
        if reserve_trials == 'Yes':
            for cv in range(cv_folds):
                formatted_training_spikes[cv], formatted_training_Behavior[cv] = \
                    format_data_from_trials(training_spikes[cv], training_Behavior[cv], n_lags)
                formatted_test_spikes[cv], formatted_test_Behavior[cv] = \
                    format_data_from_trials(test_spikes[cv], test_Behavior[cv], n_lags)
        else:
            formatted_training_spikes, formatted_training_Behavior = \
                format_data_from_trials(training_spikes, training_Behavior, n_lags)
        
        # Training the decoder
        if reserve_trials == 'Yes':
            for cv in range(cv_folds):
                H_baseline[cv] = train_wiener_filter(formatted_training_spikes[cv], formatted_training_Behavior[cv], 1)
            
                # Testing the trained decoder on testing data
                predicted_Behavior_train[cv] = test_wiener_filter(formatted_training_spikes[cv], H_baseline[cv])
                predicted_Behavior_test[cv] = test_wiener_filter(formatted_test_spikes[cv], H_baseline[cv])
        else:
            H_baseline[0] = train_wiener_filter(formatted_training_spikes, formatted_training_Behavior, 1)
            # Testing the trained decoder on testing data
            predicted_Behavior_train = test_wiener_filter(formatted_training_spikes, H_baseline[0])
            
        # Save the accuracy of the decoder
        if reserve_trials == 'Yes':
            for cv in range(cv_folds):
                r2_values_train[:,cv] = r2_score(formatted_training_Behavior[cv], \
                ...
                vaf_values_train[:,cv] = explained_variance_score(formatted_training_Behavior[cv], \
                        predicted_Behavior_train[cv], multioutput = 'raw_values')
                vaf_values_test[:,cv] = explained_variance_score(formatted_test_Behavior[cv], \
                        predicted_Behavior_test[cv], multioutput = 'raw_values')
            print(f"End of training fold {cv+1}\n")
        else:
            r2_values_train[:,0] = r2_score(formatted_training_Behavior, \
                   predicted_Behavior_train, multioutput = 'raw_values')
            vaf_values_train[:,0] = explained_variance_score(formatted_training_Behavior, \
                    predicted_Behavior_train, multioutput = 'raw_values')
                
        #%% Seperating & testing EMG by trial
        
        if reserve_trials == 'Yes':
            for cv in range(cv_folds):
                for each in zip(test_spikes[cv], test_Behavior[cv]):
                    a_, b_ = format_data_from_trials(each[0], each[1], n_lags)
                    b_pred = test_wiener_filter(a_, H_baseline[cv])
        ...
        self.r2_values_train = r2_values_train
        self.vaf_values_train = vaf_values_train
        if reserve_trials == 'Yes':
